...
def evaluate(ppl, test_data_path, batch_size, nr_data_worker=1):
    ...
    for i, (frames, t_value, scene_name, frame_range) in enumerate(val_data):
        ...
        pred,_ , _  = ppl.inference(img0, img1, time_period=t_value,
                pyr_level=PYR_LEVEL,
...
if __name__ == "__main__":
    ...
    #**********************************************************#
    # => args for model
    parser.add_argument('--pyr_level', type=int, default=6,
            help='the number of pyramid levels of ABC in testing')
    parser.add_argument('--nr_lvl_skipped', type=int, default=1,
            help='the number of skipped high-resolution pyramid levels '\
                    'of ABC in testing')
    ## test base version of ABC by default
