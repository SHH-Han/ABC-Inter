...
class MotionEstimator(nn.Module):
    ...
    def __init__(self):
        ...
        # (4*2 + 1) ** 2 + 96 * 2 + 96 + 2 = 371
        self.conv_layer1 = nn.Sequential(
                nn.Conv2d(in_channels=371, out_channels=240,
        ...
        self.conv_layer6 = nn.Sequential(
                nn.Conv2d(in_channels=96, out_channels=2,
                    kernel_size=3, stride=1, padding=1))
    ...
    def forward(self, feat0, feat1, last_feat, last_flow):
        ...
        input_feat0 = torch.cat([volume0, feat0, feat0_warp,last_feat[:,:96],last_flow[:,:2]], 1)
        input_feat1 = torch.cat([volume1, feat1, feat1_warp,last_feat[:,96:],last_flow[:,2:]], 1)
...
class ControlEstimator(nn.Module):
    ...
    def __init__(self):
        ...
        # 96+96+2=194
        self.conv_layer1 = nn.Sequential(
                nn.Conv2d(in_channels=194, out_channels=168,
                    kernel_size=1, stride=1, padding=0),
                nn.LeakyReLU(inplace=False, negative_slope=0.1))
        self.conv_layer2 = nn.Sequential(
                nn.Conv2d(in_channels=168, out_channels=144,
    ...
    def forward(self, feat0, feat1, last_feat, last_flow):
        ...
        input_feat0 = torch.cat([ feat0, last_feat[:,:96],last_flow[:,:2]], 1)
        input_feat1 = torch.cat([ feat1, last_feat[:,96:],last_flow[:,2:]], 1)
        input_feat = torch.cat([input_feat0,input_feat1],dim=0)
        feat = self.conv_layer1(input_feat)
        feat = self.conv_layer2(feat)
        feat = self.conv_layer3(feat)
        ...
        for level in list(range(pyr_level))[::-1]:
            ...
            if level == pyr_level - 1:
                ...
                last_feat = torch.zeros(
                        (N, 96*2, H // (2 ** (level+2)), W // (2 ** (level+2)))
                        ).to(img0.device)
                last_interp = None
