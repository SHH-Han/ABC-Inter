...
class MotionEstimator(nn.Module):
    ...
    def __init__(self):
        ...
        # (4*2 + 1) ** 2 + 128 * 2 + 128 + 2 = 467
        self.conv_layer1 = nn.Sequential(
                nn.Conv2d(in_channels=467, out_channels=320,
    ...
    def forward(self, feat0, feat1, last_feat, last_flow):
        ...
        input_feat0 = torch.cat([volume0, feat0, feat0_warp,last_feat[:,:128],last_flow[:,:2]], 1)
        input_feat1 = torch.cat([volume1, feat1, feat1_warp,last_feat[:,128:],last_flow[:,2:]], 1)
...
class ControlEstimator(nn.Module):
    ...
    def __init__(self):
        ...
        # 128+128+2=258
        self.conv_layer1 = nn.Sequential(
                nn.Conv2d(in_channels=258, out_channels=224,
                    kernel_size=1, stride=1, padding=0),
                nn.LeakyReLU(inplace=False, negative_slope=0.1))
        self.conv_layer2 = nn.Sequential(
                nn.Conv2d(in_channels=224, out_channels=192,
    ...
    def forward(self, feat0, feat1, last_feat, last_flow):
        ...
        input_feat0 = torch.cat([ feat0, last_feat[:,:128],last_flow[:,:2]], 1)
        input_feat1 = torch.cat([ feat1, last_feat[:,128:],last_flow[:,2:]], 1)
        input_feat = torch.cat([input_feat0,input_feat1],dim=0)
        feat = self.conv_layer1(input_feat)
        feat = self.conv_layer2(feat)
        feat = self.conv_layer3(feat)
        ...
        for level in list(range(pyr_level))[::-1]:
            ...
            if level == pyr_level - 1:
                ...
                last_feat = torch.zeros(
                        (N, 128*2, H // (2 ** (level+2)), W // (2 ** (level+2)))
                        ).to(img0.device)
                last_interp = None
