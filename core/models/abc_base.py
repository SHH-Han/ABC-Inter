...
class MotionEstimator(nn.Module):
    ...
    def forward(self, feat0, feat1, last_feat, last_flow):
        ...
        #         tenInput=feat0, tenFlow=flow[:, :2]*0.25*0.5,
        #         tenMetric=None, strType='average')
        # feat1 = softsplat.FunctionSoftsplat(
        #         tenInput=feat1, tenFlow=flow[:, 2:]*0.25*0.5,
        #         tenMetric=None, strType='average')
        # input_feat = torch.cat([feat0, feat1, last_feat], 1)
        # feat = self.conv_layer4(input_feat)
        # feat = self.conv_layer5(feat)
        # feat= self.conv_layer6(feat)

        return flow, feat
    
class ControlEstimator(nn.Module):
    """Bi-directional optical flow estimator
    1) construct partial cost volume with the CNN features from the stage 2 of
    the feature pyramid;
    2) estimate bi-directional flows, by feeding cost volume, CNN features for
    ...
    """
    def __init__(self):
        super(ControlEstimator, self).__init__()
        # (4*2 + 1) ** 2 + 64 * 2 + 64 + 4 = 277
        self.conv_layer1 = nn.Sequential(
                nn.Conv2d(in_channels=130, out_channels=112,
                    kernel_size=1, stride=1, padding=0),
                nn.LeakyReLU(inplace=False, negative_slope=0.1))
        self.conv_layer2 = nn.Sequential(
                nn.Conv2d(in_channels=112, out_channels=96,
    ...
    def forward(self, feat0, feat1, last_feat, last_flow):
        ...
        input_feat0 = torch.cat([ feat0, last_feat[:,:64],last_flow[:,:2]], 1)
        input_feat1 = torch.cat([ feat1, last_feat[:,64:],last_flow[:,2:]], 1)
        input_feat = torch.cat([input_feat0,input_feat1],dim=0)
        feat = self.conv_layer1(input_feat)
        feat = self.conv_layer2(feat)
        feat = self.conv_layer3(feat)
        B_point = self.conv_layer4(feat)

        b,_,_,_ = B_point.shape
        B_point = torch.cat([B_point[:b//2],B_point[b//2:]],dim=1)
...
class SynthesisNetwork(nn.Module):
    ...
    def get_warped_representations(self, bi_flow, bi_point, c0, c1,
            i0=None, i1=None, time_period=0.5):
        flow_0t = 2 * bi_point[:, :2] * (1 - time_period) * time_period + bi_flow[:, :2] * time_period * time_period 
        flow_1t = 2 * bi_point[:, 2:4] * (1 - time_period) * time_period + bi_flow[:, 2:4] * (1 - time_period) * (1 - time_period)
    ...
    def forward(self, last_i, i0, i1, c0_pyr, c1_pyr, bi_flow_pyr, bi_point_pyr,
            time_period=0.5):
        warped_img0, warped_img1, warped_c0, warped_c1, flow_0t_1t = \
                self.get_warped_representations(
                        bi_flow_pyr[0], bi_point_pyr[0], c0_pyr[0], c1_pyr[0], i0, i1,
        ...
        warped_c0, warped_c1 = self.get_warped_representations(
                        bi_flow_pyr[1], bi_point_pyr[1], c0_pyr[1], c1_pyr[1],
                        time_period=time_period)
        s2 = self.encoder_down2(torch.cat((s1, warped_c0, warped_c1), 1))
        warped_c0, warped_c1 = self.get_warped_representations(
                        bi_flow_pyr[2], bi_point_pyr[2], c0_pyr[2], c1_pyr[2],
                        time_period=time_period)
        ...
        # Temporal blending weights based on distance to each input frame.
        # For t in [0, 1] these are simply (1 - t) and t.
        w0 = abs(1 - time_period)
        w1 = abs(time_period)
        merged_img = (warped_img0 * refine_mask0 * w0 + \
                warped_img1 * refine_mask1 * w1)
        merged_img = merged_img / (refine_mask0 * w0 + \
                refine_mask1 * w1)
        interp_img = merged_img + refine_res
        interp_img = torch.clamp(interp_img, 0, 1)

        # Forward-warping (softsplat) leaves holes in disoccluded regions where
        # no source pixel lands.  Splatting an all-ones map with the same flow
        # yields the fraction of source coverage per output pixel; we use it to
        # mask out the holes when supervising the warped frames against the GT.
        ones0 = warped_img0.new_ones(
                warped_img0.shape[0], 1, warped_img0.shape[2], warped_img0.shape[3])
        ones1 = warped_img1.new_ones(
                warped_img1.shape[0], 1, warped_img1.shape[2], warped_img1.shape[3])
        warp_mask0 = softsplat.FunctionSoftsplat(
                tenInput=ones0, tenFlow=flow_0t_1t[:, :2].contiguous(),
                tenMetric=None, strType='average')
        warp_mask1 = softsplat.FunctionSoftsplat(
                tenInput=ones1, tenFlow=flow_0t_1t[:, 2:4].contiguous(),
                tenMetric=None, strType='average')
        ...
        extra_dict["warp_mask0"] = warp_mask0
        extra_dict["warp_mask1"] = warp_mask1
...
class Model(nn.Module):
    ...
    def forward_one_lvl(self,
            ...
            time_period=0.5, B_point=None, skip_me=False,
            control_mode="learned_control"):
        ...
        if not skip_me:
            ...
            if control_mode == "control_from_data":
                if B_point is None:
                    raise ValueError(
                        "control_from_data mode requires control points from the dataset")
            elif control_mode == "linear_motion":
                B_point = flow * 0.5
                B_point = F.interpolate(
                    input=B_point, scale_factor=4.0,
                    mode="bilinear", align_corners=False)
            elif control_mode == "learned_control":
                B_point = self.control_estimator(feat0_pyr[-1], feat1_pyr[-1],
                        feat, flow) + flow * 0.5
                B_point = F.interpolate(
                    input=B_point, scale_factor=4.0,
                    mode="bilinear", align_corners=False)
        ...
        if last_interp is None:
            flow_0t = 2 * B_point[:, :2] * (1 - time_period) * time_period + ori_resolution_flow[:, :2] * time_period * time_period 
            flow_1t = 2 * B_point[:, 2:4] * (1 - time_period) * time_period + ori_resolution_flow[:, 2:4] * (1 - time_period) * (1 - time_period)
            warped_img0 = softsplat.FunctionSoftsplat(
            ...
            w0 = abs(1 - time_period)
            w1 = abs(time_period)
            last_interp = (warped_img0 * w0 + warped_img1 * w1) / (w0 + w1)

        ## do synthesis
        interp_img, extra_dict = self.synthesis_network(
                last_interp, img0, img1, feat0_pyr, feat1_pyr, bi_flow_pyr, bi_point_pyr,
                time_period=time_period)
        extra_dict['point'] = B_point
        return flow, feat, interp_img, extra_dict

    def forward(self, img0, img1, time_period,
            B_point=None, pyr_level=None, nr_lvl_skipped=None,
            control_mode="learned_control"):
        ...
        for level in list(range(pyr_level))[::-1]:
            ...
            if level != 0:
                ...
                if B_point is not None:
                    B_point_this_lvl = F.interpolate(
                        input=B_point, scale_factor=scale_factor,
                        mode="bilinear", align_corners=False) * scale_factor
            else:
                img0_this_lvl = img0
                img1_this_lvl = img1
                if B_point is not None:
                    B_point_this_lvl = B_point
            ...
            # last level (original input resolution), only skip motion estimation
                ...
                if len(skipped_levels) == pyr_level:
                    ...
                    B_point_this_lvl = F.interpolate(
                            input=bi_points[-1], scale_factor=resize_factor,
                            mode="bilinear", align_corners=False) * resize_factor
            ...
            flow, feat, interp_img, extra_dict = self.forward_one_lvl(
                    ...
                    time_period, B_point_this_lvl, skip_me=skip_me,
                    control_mode=control_mode)
            bi_flows.append(
                    F.interpolate(input=flow, scale_factor=4.0,
                        mode="bilinear", align_corners=False))
            bi_points.append(extra_dict['point'])
        ...
        return interp_img, bi_flow,\
                {"interp_imgs": interp_imgs, "bi_flows": bi_flows,
                 ...
                 "warp_mask0": extra_dict["warp_mask0"],
                 "warp_mask1": extra_dict["warp_mask1"]}
