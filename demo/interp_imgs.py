...
def interp_imgs(ppl, ori_img0, ori_img1):
    ...
    interp_img, bi_flow, extra_dict = ppl.inference(img0, img1,
            time_period=TIME_PERIOID,
            pyr_level=PYR_LEVEL,
            control_mode=INFER_MODE)
    interp_img = interp_img[:, :, :h, :w]
    bi_flow = bi_flow[:, :, :h, :w]

    # warped (softsplat) versions of the two input frames at the target time
    warped_img0 = extra_dict["warped_img0"][:, :, :h, :w]
    warped_img1 = extra_dict["warped_img1"][:, :, :h, :w]
    ...
    warped_img0 = (warped_img0[0].clamp(0, 1) * 255).byte().cpu()\
            .numpy().transpose(1, 2, 0)
    warped_img1 = (warped_img1[0].clamp(0, 1) * 255).byte().cpu()\
            .numpy().transpose(1, 2, 0)
...
if __name__ == "__main__":
    ...
    ## load base version of ABC by default
    parser.add_argument('--model_size', type=str, default="base",
            help='model size, one of (base, large, LARGE)')
    parser.add_argument('--model_file', type=str,
            default="./checkpoints/abc_base.pkl",
            help='path to the ABC model weight')
    parser.add_argument('--infer_mode', type=str, default='learned_control',
            choices=['learned_control', 'linear_motion'],
            help='inference-time control mode passed to the model')

    ## load large version of ABC
    # parser.add_argument('--model_size', type=str, default="large",
    #         help='model size, one of (base, large, LARGE)')
    # parser.add_argument('--model_file', type=str,
    #         default="./checkpoints/abc_large.pkl",
    #         help='path to the ABC model weight')
    ...
    #         help='model size, one of (base, large, LARGE)')
    # parser.add_argument('--model_file', type=str,
    #         default="./checkpoints/abc_llarge.pkl",
    #         help='path to the ABC model weight')
